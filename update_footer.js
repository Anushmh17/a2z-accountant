...
const newFooter = `    <div class="btSiteFooter">
...
files.forEach(file => {
    try {
        ...
        const pattern = /<div class="btSiteFooter">[\s\S]*?<\/div><!-- \/btSiteFooter -->/;
        
        if (pattern.test(content)) {
            // Replace the old footer with new footer
            content = content.replace(pattern, newFooter + '<!-- /btSiteFooter -->');
            ...
            console.log(`  ✗ Could not find btSiteFooter section in ${file}`);
            failCount++;
        }
    } catch (error) {
