# Script to update footer in all HTML files
...
foreach ($file in $files) {
    if (Test-Path $file) {
        try {
            ...
            # Pattern matches from opening <div class="btSiteFooter"> to its closing </div>
            $pattern = '(?s)<div class="btSiteFooter">.*?</div>\s*</div>'
            ...
            if ($content -match $pattern) {
                ...
                Write-Host "  ✗ Could not find btSiteFooter section in $file" -ForegroundColor Yellow
                $failCount++
            }
        }
        catch {
